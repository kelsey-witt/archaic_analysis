...
**cvgtable_to_smartpca.py** takes the coverage table generated by ID_rare_AFR_archaic_genotypes.py and converts it to input files for use in smartpca.
* Usage: cvgtable_to_smartpca.py [archaic_set], where archaic_set refers to the set of archaic sites under analysis. Options for archaic_set are d_only, n_only, and arch_all, and are explained fully at the top of the script.
* Input: The panel file from the 1000 Genomes dataset and the csv file with archaic genotypes of all non-African 1000 Genomes Project individuals, generated using ID_rare_AFR_archaic_genotypes.py
* Output: 3 files necessary for input into smartpca: a .ind file (list of individuals), a .snps file (list of SNP coordinates), and a .genotypes file (genotypes for all individuals).

**archaic_site_pattern_venn.py** examines how archaic variants are shared between global regions (East Asia, Europe, and South Asia) and produces an output that can be used to generate a Venn Diagram-like figure.
* Usage: python3 archaic_venn_1kg_png.py [archaic_set], where archaic_set refers to the set of archaic sites under analysis. Options for archaic_set are na_only, nc_only, nv_only, d_only, n_only, n_shared, n_all, d_all, arch_shared, and arch_all, and are explained fully at the top of the script.
* Inputs: The csv file with presence/absence data for archaic alleles that are rare in Africa (see ID_rare_AFR_archaic_snps.py). The input file path is specified in line 35 if changes are needed.
* Output: A csv that includes list of allele sharing patterns (a 3-digit binary number where the digits represent East Asians, Europeans, and South Asians and 0/1 represents absence/presence in that population) and counts of the occurrences of each pattern
...
### Comparing archaic allele ratios using archaic SNPs to methods used by other studies
These scripts can be used to make boxplots similar to Supplemental Figure 3, to illustrate how our observation of archaic allele coverage changes with sample size match up with archaic SNPs or tracts identified in other studies, such as Steinruecken et al. 2018, Sankararaman et al. 2016, and Browning et al. 2019. All of these scripts generate coverage for Europeans and East Asians separately, but the ratio between them can easily be calculated using a method like pandas in python or tidyverse in R, allowing for the plotting of the distribution of coverage ratios as the sample size increases.

**Steinruecken_coverage_table_prep.R** takes bed files generated by Steinruecken et al. 2018 and compares ancestry tract overlap between CEU and CHB/CHS populations. It generates a table (stein_data) that includes the coverage for the populations at multiple sample sizes.

**archaic_cvg_table_sprime.py** creates a table of archaic SNPs identified using the sPrime method from Browning et al. 2019 for all non-African populations in the 1000 Genomes Phase III dataset and specifies the identity of the archaic variant (reference or alternate) and presence/absence in each individual.
* Usage: python3 archaic_cvg_table_sprime.py [archaic_set], where archaic_set refers to the set of archaic sites under analysis. Options for archaic_set are d_only, n_only, and arch_all, and are explained fully at the top of the script.
* Inputs: .panel and .vcf.gz files from 1000 Genomes Project Phase III data, and sPrime SNP calls by population from Browning et al. 2019. Input file paths are specified in the script itself and will need to be altered to fit your organization system. (Variables that need to be checked are pop_file, modern_file, and sprime_file in lines 34, 73, and 77).
* Output: A csv file where each row is a site that has an allele that is identified as archaic by sPrime, and the columns include position information, reference/alternative/and archaic allele identities, and presence/absence of the allele in non-African individuals
* This csv file is identical in format to the one produced by ID_rare_AFR_archaic_snps.py, so you can use the above script archaic_mp_cvg_ss.py to generate a coverage table that can be used for plotting changes in allele coverage over time.

**sank_interval_summing.py** looks at archaic ancestry tract data from Sankararaman et al. 2016 and uses it to plot increases in archaic coverage with increasing sample size in Europeans and East Asians 
* Usage: python3 sank_interval_summing.py
* Inputs: .ind and .haplotypes files generated from Sankararaman et al. 2016. Input file paths are specified in the script itself and will need to be altered to fit your organization system. (Variables that need to be checked are indFile in line 48 and hapFile in line 68)
* Output: A csv file with the summed total of archaic tract lengths in a given sample size of individuals from "eastasia" and "westeurasia". The file contains all regions, all sample sizes, and both neanderthal- and denisovan-specific tracts, with 100 replicates of each population/sample/archaic source combination. 
