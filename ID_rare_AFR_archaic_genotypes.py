...
and Denisovan). It records the genotype for each non-African individual.

The input files are the csv files for each chromosome containing alleles that are rare in Africa 
(which come from ID_rare_AFR_genotypes.py, "afrInfile", line 62), and the four archaic human VCF
...
columns are the genotypes of each non-African individual, ordered by population. The header lists the 
...
outfile = "archaic_ancestral_rareAFR_genotypes.csv"
...
with open(outfile, 'w', newline = '') as csvfile:
    ...
    for chromosome in chr_list:
        afrInfile = "rare_AFR_genotypes_ancestral_chr" + chromosome + ".csv.gz"
