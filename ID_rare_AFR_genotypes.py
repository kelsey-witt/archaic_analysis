...
population. It records the genotype for each non-African individual.

The input files are the .panel file from the 1000 Genomes dataset ("pop_file", line 24), and the VCF
files of each chromosome from the 1000 Genomes Phase III Dataset ("modern_file", lines 107 and 109).

Usage: python3 ID_rare_AFR_genotypes.py

This script generates a csv file for each chromosome, currently named "rare_AFR_genotypes_chrN.csv", where
N is 1-22 or X ("outfile", line 110). Each row represents a site in the genome, and the first five columns
are the chromosome number, position, the reference allele, the alternate allele, and the allele that is rare
in Africa (0 if reference, 1 if alternate). The remaining columns are the genotypes of each non-African 
individual, ordered by population. The header lists the individual's ID as their population and the 0-indexed 
column number they appear in the 1000 Genomes vcf, separated by an underscore (ie "GBR_9").
"""
...
for chromosome in chr_list:
    ...
    with gzip.open(modern_file, 'rt') as mf:
        outfile = "rare_AFR_genotypes_chr" + chromosome + ".csv"
